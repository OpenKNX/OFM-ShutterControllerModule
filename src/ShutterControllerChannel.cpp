...
void ShutterControllerChannel::processInputKo(GroupObject &ko)
{
    ...
    case SHC_KoCWindowOpenOpened2:
        ...
        {
            if (index == SHC_KoCWindowOpenOpened1)
            {
...
bool ShutterControllerChannel::processCommand(const std::string cmd, bool diagnoseKo, bool &diagnosticLogLoopRequest)
{
    ...
    {
        KoSHC_CManualUpDownWithoutSpecialFunction.valueNoSend(false, DPT_UpDown);
        processInputKo(KoSHC_CManualUpDownWithoutSpecialFunction);
        return true;
    }
    else if (cmd == "mv-")
    {
        KoSHC_CManualUpDownWithoutSpecialFunction.valueNoSend(true, DPT_UpDown);
        processInputKo(KoSHC_CManualUpDownWithoutSpecialFunction);
        return true;
    }
    else if (cmd == "m-")
...
void ShutterControllerChannel::execute(CallContext &callContext)
{
    ...
    if (_waitForWindowOpenEvalulation != 0)
    {
        ...
        if (callContext.currentMillis - _waitForWindowOpenEvalulation >= waitTime)
        {
            ...
            {
                case 0:
                    if (((bool) KoSHC_CWindowOpenOpened2.value(DPT_OpenClose)) == !ParamSHC_CWindowOpenContactInvert1 && 
                        ((bool)KoSHC_CWindowOpenOpened1.value(DPT_OpenClose)) != !ParamSHC_CWindowOpenContactInvert2)
